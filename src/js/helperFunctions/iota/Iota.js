...
export default class Iota {
	 ...
	 */
	static generateSurveyAddress(tag = 'DVOTE') {
		const now = new Date().getTime();
		const random = Math.floor(Math.random() * 1000000 + 1);
		const trytes = `${tag}IOTASURVEYADDRESS${Trytes.encodeTextAsTryteString(
			now.toString()
		)}${Trytes.encodeTextAsTryteString(random.toString()) + Trytes.encodeTextAsTryteString(now.toString())}`;
	...
	vote(tryteAddress, voteObj) {
		...
		return this.send(tryteAddress, tryteMessage, this.tag);
	}
		...
		return new Promise((resolve, reject) => {
			this.iotaNode.api.getBundle(transaction, (error, sucess2) => {
				if (error) {
					...
					const [usedMessage] = message.split('9999999999999999999999999999999999999');
					let obj;
